...
impl<'model_manager> Environment<'model_manager> {
    ...
    pub fn get_data_from_db(&self, model_name: &str, id: u32) -> Result<MapOfFields, Box<dyn Error>> {
        ...
        Err(RecordNotFoundError { model_name: model_name.to_string(), id }.into())
    }
    ...
    pub fn insert_data_to_db(&mut self, model_name: &str, data: &MapOfFields) -> Result<u32, Box<dyn Error>> {
    ...
    pub fn save_record<M>(&mut self, record: &M) where M: Model {
        ...
        let data = record.get_data();
        self.cache.insert_record_model_with_map(model_name.as_str(), id, data);
    }
    ...
    pub fn get_record_from_name(&mut self, model_name: &str, id: u32) -> Result<Box<dyn Model>, Box<dyn Error>> {
    ...
    ///
    /// If the record is not present in cache, loads it from the database
    pub fn get_record_from_internal_model(&mut self, internal_model: &InternalModel, id: u32) -> Result<Box<dyn Model>, Box<dyn Error>> {
        let map_of_fields = self.get_map_of_field(internal_model.name.as_str(), id)?;
        Ok(self.model_manager.create_instance_from_internal_model(id, map_of_fields, internal_model))
    }

    /// Returns an instance of given model for a specific id
    ///
    /// If the record is not present in cache, loads it from the database
    pub fn get_record<M>(&mut self, id: u32) -> Result<M, Box<dyn Error>> where M: Model + 'static {
        let model_name = M::get_model_name();
    ...
    fn get_map_of_field(&mut self, model_name: &str, id: u32) -> Result<MapOfFields, Box<dyn Error>> {
        ...
        if cache_record.is_none() {
            return Err(cache::errors::RecordNotFoundError { model_name: model_name.to_string(), id }.into());
        }
    ...
    pub fn create_new_record_from_map<M>(&mut self, data: &mut MapOfFields) -> Result<M, Box<dyn Error>> where M: Model + 'static {
        let model_name = M::get_model_name();
    ...
    /// Create a new record for a specific model and a given list of fields
    pub fn create_record_from_name(&mut self, model_name: &str, data: &mut MapOfFields) -> Result<Box<dyn Model>, Box<dyn Error>> {
        let id = self._create_new_record(model_name, data)?;
        self.get_record_from_name(model_name, id)
    }

    fn _create_new_record(&mut self, model_name: &str, data: &mut MapOfFields) -> Result<u32, Box<dyn Error>> {
        ...
        if let Some(missing_fields) = missing_fields {
            ...
            if let Some(final_internal_model) = final_internal_model {
                let computed_fields: Vec<String> = missing_fields.into_iter().filter(|f| final_internal_model.is_computed_field(f)).collect();
    ...
    pub fn fill_default_values_on_map(&self, model_name: &str, data: &mut MapOfFields) -> Option<Vec<String>> {
        let final_internal_model = self.model_manager.get_model(model_name)?;
